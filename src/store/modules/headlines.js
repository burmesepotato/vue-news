...
export const actions = {
  ...
  getCurrentHeadline({ commit }) {
    const headlineString = localStorage.getItem(LS_NAME);
    if (headlineString && headlineString.length) commit('SET_CURRENT_HEADLINE', JSON.parse(headlineString));
  },
...
export const getters = {
  filteredHeadlines: (state, getters, rootState) => {
    ...
    if (rootState.sources.selectedSource) {
      results = results.filter((headline) => headline.source.name.toLowerCase() === rootState.sources.selectedSource.toLowerCase());
    }
    return results;
  },
};
